...
const Recordings = () => {
  ...
  const fetchRecordings = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await recordingAPI.getRecordingsBySchedule(platformId, scheduleId);
                ...
                <div key={rec._id} className="rounded-lg border border-gray-200 p-4 hover:shadow transition">
                  <div className="min-w-0">
                    ...
                    <div className="text-base font-medium text-gray-900 truncate">
                      {rec.occurrenceId ? `Occurrence ${rec.occurrenceId.slice(-6)}` : rec._id}
                    </div>
                    {rec.createdAt && (
